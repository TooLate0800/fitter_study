...
double chi2(const double * fitpara){
  ...
  for(int i=0;i<ndata;i++){
    ...
    GE_cal=GE_fun(zv,R,pd,pd1,pd2,fp1);
...
for (int k=1; k<2; k++){
  ...
  //for (int j = 1; j <  10; j++){
     ...
     for(int i = 0; i <  1; i++){
      ...
      string fname1="../../data/Carl-norm_0.06GeV.dat";      
      //string fname1=Form("../../data/Carl-norm_0.%dGeV.dat",k);      
      ...
      cout <<"0."<<k<<"GeV:"<< "chi2fit= " << chi2fit << " , Rfit= " << Rfit << " , Rfiterr= " << Rfiterr << endl;
      
      ofstream outFile1;
      //outFile1.open(Form("fit_result_PRad_range/z4_Model%d_1e4.txt",j), std::ios_base::app);
      outFile1.open(Form("fit_result/z4_Cut0%dGeV_Model%d_1e4.txt",k,j), std::ios_base::app);
      //outFile1<<Rfit<<endl;
