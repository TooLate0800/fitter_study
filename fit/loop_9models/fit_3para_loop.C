...
int xyminimizer(const char * minName = "Minuit", const char * algoName = ""){
  ...
  //step[0]=0.01;step[1]=0.01;step[2]=0.1;step[3]=0.01;//step[4]=0.00001;
  //variable[0]=0.87;variable[1]=9.19364e-02;variable[2]=1.0;variable[3]=0.001;//variable[4]=9.19364e-02;
  step[0]=0.001;step[1]=0.01;step[2]=0.1;step[3]=0.1;//step[4]=0.00001;
  variable[0]=0.84;variable[1]=-2.88973e-02;variable[2]=1.0;variable[3]=-3.15945e-02;//variable[4]=9.19364e-02;
...
for (int k = 1; k <  10; k++){
  for (int j = 1; j <  10; j++){
       for(int i = 0; i <  10000; i++){
        ...
        string fname1=Form("/var/phy/project/mepg/jz271/fitter_study/generator/Mainz_pseudo_Cut0%dGeV/Model_%d_table_%d.txt",k, j,i+1);//input files 
        double modiFactor = 1.;
        ...
        //if (chi2fit < 538){
            outFile1.open(Form("fit_result/R12_Cut0%dGeV_Model%d_1e4.txt",k,j), std::ios_base::app);
            outFile1<<Rfit<<endl;
            outFile1.close();
            outFile2.open(Form("fit_result/R12_Cut0%dGeV_Model%d_1e4_all.txt",k,j), std::ios_base::app);
