...
double GE_fun(const double &Q2, const double &R, const double &pd, const double& pd1, const double &fp)
//double GE_fun(const double &z, const double &R, const double &pd, const double& pd1, const double &fp)
{
  ...
  double R2=R*R;
   ...
   GE=fp*(1.-R2*Q2/6.+pd*Q2)/(1.+pd*Q2+pd1*Q2*Q2);
...
double chi2(const double * fitpara){
  ...
  for(int i=0;i<ndata;i++){
    ...
    GE_cal=GE_fun(Q2v,R,pd,pd1,fp1);
  ...
  */
  for (int j = 1; j <  10; j++){
     ...
     for(int i = 0; i <  10000; i++){
      ...
      ofstream outFile1;
      outFile1.open(Form("fit_result_PRad_range/R12_test_Model%d_1e4.txt",j), std::ios_base::app);
      outFile1<<Rfit<<endl;
      outFile1.close();
      }
   }
