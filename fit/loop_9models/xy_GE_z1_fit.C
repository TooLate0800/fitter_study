...
double GE_fun(const double &z, const double &R, const double &fp)
{
  double GE;
  double R2=R*R;

  // GE=fp*(1.-R2*Q2/6.+pd*Q2)/(1.+pd*Q2);
  GE=fp*(1. - R2*z*2.0*2./3.);
...
int xyminimizer(const char * minName = "Minuit", const char * algoName = ""){
  ...
  step[0]=0.001;step[1]=0.0001;//step[1]=0.0001;//step[3]=0.0001;step[4]=0.0001;
  variable[0]=0.87;variable[1]=1.0;//variable[2]=1.0;//variable[3]=1.0;variable[4]=1.0;
  ...
  */
  for (int j = 1; j <  10; j++){
     ...
     for(int i = 0; i <  10000; i++){
      ...
      ofstream outFile1;
      outFile1.open(Form("fit_result_PRad_range/z1_Model%d_1e4.txt",j), std::ios_base::app);
      outFile1<<Rfit<<endl;
      outFile1.close();
    }
   }
