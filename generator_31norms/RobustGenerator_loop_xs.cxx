...
for (int k=7; k<8;k++){
    ...
    for (int i=0; i<34; i++){//sets
        inFile[i].open(Form("../data/cs_data/%d.txt",i));
        inFile[i]>>count[i];
...
//            }//dGE is taken from the smallest diff one
    ...
    for (int n=0; n<1000; n++){//repeat for X times/ generate X sets of pseudo-data
        if (n%10 == 0) cout<<n<<endl;
        int GMFF = gRandom->Integer(9);
        for (int i=0; i<34; i++){//sets
            ofstream outFile;
            outFile.open(Form("xs_pseudo_Bernauer_xs_1e3/Model%d_norm%d_table_%d.txt",k+1, i+1, n+1));//define the output file name
            ...
            for (int j=0; j<count[i]; j++){
                ...
                double GM = GetGM(Q2_GeV[i][j],5);//Random
