...
for (int k=0; k<9;k++){
    ...
    for (int n=0; n<10000; n++){//repeat for X times/ generate X sets of pseudo-data
        ...
        for (int i=0; i<34; i++){//sets
            ofstream outFile;
            outFile.open(Form("Pseudo_31/Model%d_norm%d_table_%d.txt",k+1, i+1, n+1));//define the output file name
            ...
            for (int j=0; j<count[i]; j++){
                ...
                outFile<<Q2_GeV[i][j]*tofm<<" "<<thisGE<<" "<<dGE[i][j]<<endl;//write the pseudo-data to the files
            }
            outFile.close();
        }
    }
