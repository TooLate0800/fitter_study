...
int xyminimizer(const char * minName = "Minuit", const char * algoName = ""){
  ...
  min->SetLimitedVariable(0, "R_E", variable[0], step[0], 0.6, 1.0);
  min->SetLimitedVariable(1, "pb_E1", 1.45487683e+01,  0.01, -1e4, 1e4);
  min->SetLimitedVariable(2, "pb_E2", -8.87959239e+01, 0.01, -1e4, 1e4);
  min->SetLimitedVariable(3, "pb_E3", 4.61097705e+02,  0.01, -1e4, 1e4);
  min->SetLimitedVariable(4, "pb_E4", -1.67562381e+03, 0.01, -1e4, 1e4);
  min->SetLimitedVariable(5, "pb_E5", 4.07646487e+03,  0.01, -1e4, 1e4);
  min->SetLimitedVariable(6, "pb_E6", -6.45411460e+03, 0.01, -1e4, 1e4);
  min->SetLimitedVariable(7, "pb_E7", 6.34035079e+03,  0.01, -1e4, 1e4);
  min->SetLimitedVariable(8, "pb_E8", -3.49373923e+03, 0.01, -1e4, 1e4);
  min->SetLimitedVariable(9, "pb_E9", 8.22601568e+02,  0.01, -1e4, 1e4);
  //for (int j=1; j<10; j++){
  //    min->SetLimitedVariable(j, Form("pb_E%d",j), 9.19364e-02, 0.1, -1e4, 1e4);
  //}
  min->SetLimitedVariable(10, "R_M", variable[2], step[2], 0.6, 1.0);
  min->SetLimitedVariable(11, "pb_M1", 1.0222000, 0.01, -1e4, 1e4);
  min->SetLimitedVariable(12, "pb_M2", 23.494500, 0.01, -1e4, 1e4);
  min->SetLimitedVariable(13, "pb_M3", -93.03720, 0.01, -1e4, 1e4);
  min->SetLimitedVariable(14, "pb_M4", 140.79840, 0.01, -1e4, 1e4);
  min->SetLimitedVariable(15, "pb_M5", -0.365600, 0.01, -1e4, 1e4);
  min->SetLimitedVariable(16, "pb_M6", -305.6759, 0.01, -1e4, 1e4);
  min->SetLimitedVariable(17, "pb_M7", 444.62510, 0.01, -1e4, 1e4);
  min->SetLimitedVariable(18, "pb_M8", -273.6688, 0.01, -1e4, 1e4);
  min->SetLimitedVariable(19, "pb_M9", 64.581100, 0.01, -1e4, 1e4);
  //for (int j=1; j<10; j++){
  //    min->SetLimitedVariable(j+10, Form("pb_M%d",j), 9.19364e-02, 0.1, -1e4, 1e4);
  //}

  for (int j=1; j<32; j++){
      min->SetLimitedVariable(j+19, Form("fp_%d",j), 1.0, 0.001, 0.9, 1.1);
  }
  min->Minimize();
...
int main(Int_t argc, char *argv[]){
  ...
  string tablename = "../data/cs_data/Cut_01GeV/norm_table.dat";
  readtable(tablename);
  ...
  for (int j = 0; j <  9; j++){//models
       for(int i = 0; i <  1000; i++){//table
           ...
           for (int l=0; l<34; l++){
               fname[l] = Form("../generator_31norms/xs_pseudo_GMfromRatio_CutQ2_01GeV/Model%d_norm%d_table_%d.txt",j+1, l+1, i+1); 
           ...
           outFile1.open(Form("result/poly10_xsfit_RE_GMfromRatio_Cut01GeV_Model%d.txt",j+1), std::ios_base::app);
           //outFile1.open(Form("result/R11_xsfit_RE_GMRandom_Model%d.txt",j+1), std::ios_base::app);
           outFile1<<REfit<<endl;
           outFile1.close();
           outFile2.open(Form("result/poly_xsfit_all_GMfromRatio_Cut01GeV_Model%d.txt",j+1), std::ios_base::app);
