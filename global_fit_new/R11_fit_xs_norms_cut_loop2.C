...
int main(Int_t argc, char *argv[]){
  ...
  for (int j = 0; j <  9; j++){//models
       for(int i = 1; i <  10000; i++){//table


               string tablename = "../data/cs_data/Cut_06GeV/norm_table.dat";
               readtable(tablename); 


               for (int l=0; l<number_of_sets; l++){
                   fname[l] = Form("../generator_31norms/xs_pseudo_GMfromRatio_CutQ2_06GeV_1e4/Model%d_norm%d_table_%d.txt",j+1, l+1, i+1);
                   readdata(fname[l],l);
               ...
               outFile1.open(Form("result/R11_xsfit_RE_GMfromRatio_Cut_06GeV_1e4_Model%d.txt",j+1), std::ios_base::app);
               outFile1<<Rfit<<endl;
               outFile1.close();
               outFile2.open(Form("result/R11_xsfit_all_GMfromRatio_Cut_06GeV_1e4_Model%d.txt",j+1), std::ios_base::app);
