...
int xyminimizer(const char * minName = "Minuit", const char * algoName = ""){
  ...
  for (int j=1; j<32; j++){
      min->SetLimitedVariable(j+3, Form("fp_%d",j), 1.0, 0.001, 0.8, 1.1);
  }
  min->Minimize();
...
int main(Int_t argc, char *argv[]){
  ...
  string fname[34];
  for (int j = 1; j <  2; j++){//models
       for(int i = 0; i <  1; i++){//table


           for (int l=0; l<34; l++){
               fname[l] = Form("../generator_31norms/xs_pseudo_GMfromRatio_CutQ2_07GeV/Model%d_norm%d_table_%d.txt",j+1, l+1, i+1); 
           ...
           outFile1.open(Form("result/R11_xsfit_RE_GMfromRatio_check_Model%d.txt",j+1), std::ios_base::app);
           //outFile1.open(Form("result/R11_xsfit_RE_GMRandom_Model%d.txt",j+1), std::ios_base::app);
           //outFile1<<REfit<<endl;
           outFile1.close();
           outFile2.open(Form("result/R11_xsfit_all_GMfromRatio_check_Model%d.txt",j+1), std::ios_base::app);
