...
double sig_fun(const double &theta, const double &Q2, const double &z, const double &R, const double &pb_E, const double &pa_M, const double &pb_M, const double &pd1_E, const double &pd1_M, const double &fp)
//double sig_fun(const double &theta, const double &Q2, const double &z, const double &R, const double &pb_E, const double &pa_M, const double &pb_M, const double &pd1_E, const double &pd2_E, const double &pd1_M, const double &pd2_M, const double &fp)
{
  double R2=R*R;
  ...
  //double GE=(1. - R2*z*2.0*2./3.+pb_E*z*z + pd1_E*z*z*z+ pd2_E*z*z*z*z);
  //double GM=mu*(1. +pa_M*z+pb_M*z*z + pd1_M*z*z*z+ pd2_M*z*z*z*z);
  double GE=(1. - R2*z*2.0*2./3.+pb_E*z*z + pd1_E*z*z*z);
  double GM=mu*(1. +pa_M*z+pb_M*z*z + pd1_M*z*z*z);
...
double chi2(const double * fitpara){
  ...
  for (int l = 0; l<34; l++){
      for(int i=0;i<ndata[l];i++){
        thetav=theta[l][i];zv=z_arr[l][i];Q2v=Q2[l][i];cs0=cs[l][i];csd=dcs[l][i];
        cs_cal=sig_fun(thetav,Q2v, zv,R,pb_E, pa_M, pb_M, pd1_E, pd1_M, param_list[l]);
        //cs_cal=sig_fun(thetav,Q2v, zv,R,pb_E, pa_M, pb_M, pd1_E, pd2_E, pd1_M, pd2_M, param_list[l]);
        //cs_cal=sig_fun(thetav,Q2v,R,pb_E, pa_M, pb_M, pd1_E, pd2_E, pd1_M, pd2_M, param_list[l]);
...
int xyminimizer(const char * minName = "Minuit", const char * algoName = ""){
  ...
  min->SetLimitedVariable(1, "pb_E", variable[1], step[1], -5000., 5000.);
  min->SetLimitedVariable(2, "pa_M", variable[2], step[2], -5000., 5000.);
  min->SetLimitedVariable(3, "pb_M", variable[3], step[3], -5000., 5000.);
  min->SetLimitedVariable(4, "pd1_E", 9.19364e-02, 0.001,  -5000., 5000.);
  min->SetLimitedVariable(5, "pd2_E", 9.19364e-02, 0.001,  -5000., 5000.);
  //min->SetLimitedVariable(6, "pd1_M", 9.19364e-02, 0.001,  -5000., 5000.);
  //min->SetLimitedVariable(7, "pd2_M", 9.19364e-02, 0.001,  -5000., 5000.);

  for (int j=1; j<32; j++){
      min->SetLimitedVariable(j+5, Form("fp_%d",j), 1.0, 0.001, 0.7, 1.3);
  }
  min->Minimize();
...
int main(Int_t argc, char *argv[]){
  ...
  for (int i=0; i<34; i++){
      fname[i] = Form("../generator_31norms/xs_pseudo_GMfromRatio_Model8/Model8_norm%d_table_1.txt",i); 
      //fname[i] = Form("../generator_31norms/xs_pseudo_GMfromRatio/Model8_norm%d_table_1.txt",i); 
