{
    ...
    gr[2]->GetXaxis()->SetLimits(0.0, 1.01);
    gr[2]->GetYaxis()->SetRangeUser(0.4, 1.0);
    ...
    TLegend* leg = new TLegend(0.23, 0.23, 0.90, 0.53);
    ...
    leg->AddEntry(gr[1], "Rational (1,1) fit with RS G_{E}^{p} data", "ep");
    leg->AddEntry(gr[3], "Polynomial-Z (2) fit with normalized G_{E}^{p} data", "ep");
    leg->AddEntry(gr[4], "polynomial-Q^{2} (10) fit with #sigma_{exp}/#sigma_{dipole} data", "ep");
